<div>
    ...
    @else
        <div class="flex flex-col gap-4">
            ...
            @if ($isSignedIn)
                <div class="flex items-center justify-between gap-4 rounded-xl border border-gray-100 dark:border-gray-900 p-4">
                    <p class="text-sm text-gray-600 dark:text-gray-300">
                        Signed in as <span class="font-medium text-black dark:text-white">{{ $customerName }}</span>
                    </p>
                    <button type="button"
                            wire:click="submit"
                            class="rounded-md bg-primary hover:bg-primary-600 px-5 py-2.5 text-sm font-medium text-white transition">
                        Request a quote
                    </button>
                </div>
            @else
                <div class="rounded-xl border border-gray-100 dark:border-gray-900 p-6 flex flex-col gap-4">
                    ...
                    <form wire:submit="signIn" class="flex flex-col gap-3 max-w-md">
                        <input type="email"
                               wire:model="email"
                        ...
                        @enderror
                        <button type="submit"
                                class="rounded-md bg-primary hover:bg-primary-600 px-4 py-2 text-sm font-medium text-white transition">
                            Sign in
                        </button>
                    </form>

                    <div class="flex flex-col sm:flex-row gap-2 text-sm text-gray-600 dark:text-gray-300">
                        ...
                        <a href="{{ url()->getCustomerPortalUrl('login') }}" class="font-medium text-primary hover:underline">Go to the customer portal</a>
                    </div>
                </div>
            @endif
        </div>
