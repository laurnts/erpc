@php
    /** @var \App\Models\GoodsReceiveBatch|null $record */
    $record = $record ?? $getRecord();
    $mediaItems = $record ? $record->getMediaRecords() : collect();
    $request = $record?->request;
@endphp

@if($mediaItems->isNotEmpty())
    <div class="space-y-2">
        <p class="text-sm font-medium text-gray-700 dark:text-gray-300">Goods Receive Documents</p>
                        ...
                        <button
                            ...
                            onclick="
                                if (confirm('Are you sure you want to delete this document?')) {
                                    fetch('{{ route('requests.goods-receive.delete', ['request' => $request->id, 'media' => $media->id]) }}', {
                                        method: 'DELETE',
                                        headers: {
...
@else
    <p class="text-sm text-gray-500 dark:text-gray-400">No documents in this batch.</p>
@endif
