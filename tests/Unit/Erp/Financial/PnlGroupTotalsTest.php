...
it('uses net sell (line_subtotal) as the P&L margin base, never gross', function (): void {
    ...
    $totals = BuyerQuoteItem::collectTotals(new Collection([$item]), isServiceRequest: false);
...
it('excludes service child items from P&L group totals', function (): void {
    ...
    $totals = BuyerQuoteItem::collectTotals(new Collection([$main]), isServiceRequest: true);
