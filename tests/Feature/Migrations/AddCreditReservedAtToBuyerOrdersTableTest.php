...
/**
 * Production holds confirmed orders whose only history row was stamped by an
 * older code path with transaction_type 'used' rather than 'debit', for the
 * same "order confirmed" event. Those orders are still confirmed with
 * credit_released = 0.00 — live exposure — so the backfill must not skip them.
 */
it('backfills credit_reserved_at from a legacy used-stamped history row', function (): void {
    $team = Team::factory()->create();
    $currency = Currency::factory()->create();
    $buyer = Company::factory()->buyer()->recycle($team)->create();
    $request = Request::factory()->recycle($team)->recycle($buyer)->create();
