...
final class SupplierOrderExporter extends BaseExporter
{
    ...
    public static function getColumns(): array
    {
        return [
            ...
            ExportColumn::make('base_total')->label('Total (Base)'),
            ExportColumn::make('expected_delivery_date')->label('Expected Delivery')->formatStateUsing(fn (?Carbon $state): string => $state?->format('Y-m-d') ?? ''),
    ...
    {
        $body = 'Your supplier orders export has completed and '.Number::format($export->successful_rows).' '.str('row')->plural($export->successful_rows).' exported.';
