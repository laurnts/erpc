...
### Requirement: Supplier Request Send Visibility Gate
The system SHALL record when a supplier quote solicitation was actually sent to the supplier, and SHALL expose supplier requests to the supplier portal only after that send; auto-generated pending quotes without a send remain internal.
...
#### Scenario: Unsent supplier requests are invisible to suppliers
...
The system SHALL allow the supplier to submit their quotation for a sent supplier request from the supplier portal, performing the same write as the internal price-entry flow, with the exchange rate always resolved server-side.
...
The system SHALL let a supplier decline a sent supplier request; declined quotes keep their pending status but are excluded from reminders and expiry, and a staff re-send resets the decline.
...
- **WHEN** staff re-send the request to a supplier whose quote row has `declined_at` set
- **THEN** `declined_at` and the `submitted_*` fields are cleared and `sent_to_supplier_at` is re-stamped
- **AND** the portal shows it as a fresh open request

### Requirement: Supplier Request Outcome Announcement
