...
## 1. Database Schema
- [ ] 1.1 Create migration to add `request_type` column to `requests` table (enum: goods, service, default: goods)
- [ ] 1.2 Create migration to add `parent_id` column to `request_items` table (nullable foreign key to request_items)
- [ ] 1.3 Create migration for `acceptance_reports` table with fields:
  - id, request_id, report_number, reported_at, reported_by, notes, created_at, updated_at, deleted_at
- [ ] 1.4 Create migration for `acceptance_report_items` table (pivot for request items)
- [ ] 1.5 Add indexes for performance (request_type, parent_id)

## 2. Models and Enums
- [ ] 2.1 Create `RequestType` enum (Goods, Service)
- [ ] 2.2 Update `Request` model:
...
- [ ] 3.3 Create `AcceptanceReportResource`:
...
- [ ] 3.4 Add `AcceptanceReportsRelationManager` to `RequestResource`:
...
## 5. Business Logic
- [ ] 5.1 Update `RequestItem` form validation:
...
- [ ] 6.1 Add visual indicator in Request list/table for request type
- [ ] 6.2 Update Request view page to show appropriate tabs based on request type
- [ ] 6.3 Add helper text in forms explaining Service vs Goods differences
- [ ] 6.4 Style child items differently in RequestItem table (indentation or badge)

## 7. Testing
- [ ] 7.1 Write feature tests for Service request creation
- [ ] 7.2 Write tests for child items creation and management
- [ ] 7.3 Write tests for acceptance report creation and file upload
- [ ] 7.4 Write tests for workflow differences (no quotation evaluation, no inbound shipments)
- [ ] 7.5 Write tests for backward compatibility (existing Goods requests still work)
- [ ] 7.6 Write tests for form conditional logic

## 8. Documentation
- [ ] 8.1 Update README.md with Service request type information
- [ ] 8.2 Document acceptance report workflow
- [ ] 8.3 Document child items structure and usage
