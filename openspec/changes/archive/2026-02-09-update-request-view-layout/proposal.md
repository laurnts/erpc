...
- Updated `ViewRequest::infolist()` to use 3-column grid layout
- Removed "Requested Items" section
- Added helper methods:
  - `getDisplayPrepayment()` - Formats prepayment value
  - `getPaymentTermsList()` - Generates payment terms HTML table
  - `getShipmentsList()` - Generates shipments HTML table
  - `getPrimaryBuyerOrder()` - Gets primary buyer order
  - `isPaymentTermPaid()` - Determines payment term status
  - `mapFilamentColorToTailwind()` - Maps Filament colors to Tailwind classes
